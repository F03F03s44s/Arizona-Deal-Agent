...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m app",
        description=(
            "Rank Phoenix Craigslist deals by blending profit margin with "
        ...
        epilog=(
            ...
            "  python -m app --query macbook --refresh\n"
        ),
    ...
    parser.add_argument(
        "--query",
        default=None,
        help="Optional Craigslist search query.",
    )
    parser.add_argument(
        "--refresh",
        action="store_true",
        help="Bypass the scrape cache and fetch fresh listings.",
    )
    parser.add_argument(
        "--json",
        action="store_true",
...
def run(argv: Sequence[str] | None = None) -> str:
    """Parse arguments, scrape/rank deals, and return the output text."""
    ...
    deals, source = load_deals(query=args.query, refresh=args.refresh)
    result = rank_deals(deals, args.budget, args.profit_weight)
    if args.json:
        return result.model_dump_json(indent=2)
    report = format_report(result)
    return f"Source: {source} ({len(deals)} deals)\n{report}"
