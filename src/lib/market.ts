...
export const MARKET_COMPS: MarketComp[] = [
  ...
  { id: "patio-umbrella", pattern: /\b(patio|market)\s+umbrella\b/i, category: "outdoor", marketPrice: 180, resaleHaircut: 0.7 },
  ...
  { id: "solar-lights", pattern: /\bsolar (string )?(light|lamp|post)\b/i, category: "solar", marketPrice: 45, resaleHaircut: 0.6 },
  ...
  { id: "house", pattern: /\b(\d\s*bed|\d\/\d|single[- ]family|house|home|townhome|duplex|condo)\b/i, category: "housing", marketPrice: 385000, resaleHaircut: 0.97 },
];

export function matchMarket(text: string): MarketComp | null {
  for (const comp of MARKET_COMPS) {
    if (comp.pattern.test(text)) return comp;
  }
  return null;
}

export function inferCategory(text: string): Category {
  return matchMarket(text)?.category ?? "other";
}
